...
import { ZstdCompression, CompressionOptions, CompressionResult } from './zstd-compression';

export interface ParallelOptions extends CompressionOptions {
  workerCount?: number;      // Number of workers (default: CPU cores)
  taskTimeout?: number;      // Task timeout in ms (default: 30000)
  enableFallback?: boolean;  // Enable fallback to single-thread (default: true)
}
 ...
 */
export class CompressionParallel extends EventEmitter {
  private options: Required<ParallelOptions>;
  private workers: WorkerState[] = [];
  private taskQueue: Task[] = [];
  private activeTasks: Map<number, Task> = new Map();
  private taskIdCounter = 0;
  private singleThreadCompressor: ZstdCompression;
  ...
  constructor(options: ParallelOptions = {}) {
    ...
    this.singleThreadCompressor = new ZstdCompression({
   ...
   */
  private handleWorkerMessage(state: WorkerState, result: any): void {
   ...
   * Add task to queue
   */
  private addTask(action: 'compress' | 'decompress', data: Buffer): Promise<Buffer> {
   ...
   */
  async compressParallel(data: Buffer): Promise<Buffer> {
    if (this.terminated) {
    ...
    const chunkCount = this.singleThreadCompressor.calculateChunkCount(
      data.length,
   ...
   */
  async decompressParallel(data: Buffer): Promise<Buffer> {
    if (this.terminated) {
      throw new Error('CompressionParallel has been terminated');
    }
    ...
    const decompressedChunks = await Promise.all(
      chunks.map(chunk => this.addTask('decompress', chunk))
    );
   ...
   */
  private combineChunks(compressed: Buffer[], original: Buffer[]): Buffer {
   ...
   */
  private splitCombinedChunks(data: Buffer): Buffer[] {
    const chunks: Buffer[] = [];
    let offset = 0;
   ...
   */
  async simulateWorkerCrash(): Promise<void> {
...
export const compressionParallel = new CompressionParallel();
